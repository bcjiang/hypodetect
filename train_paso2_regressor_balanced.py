...
class RegressorDatasetTen(Dataset):
    """ 
    ...
    def __getitem__(self, idx):
        ...
        # Preprocessing        
        img = img/np.max(img)
        sO2_map = np.expand_dims(sO2_map,0)
        img = np.concatenate((img, sO2_map),axis=0)
...
if __name__ == '__main__':
    ...
    results_root = "./results/exp_"+"paso2_regressor_balanced"
    ...
    for test_group_idx in range(0, 10):
        ...
        # load data
        train_set = RegressorDatasetTen(data_path="./dataset/data_point_tensets_good.pk", random_seed=0, mode='train', test_group=test_group_idx, inputSelect='FullPA')
        test_set = RegressorDatasetTen(data_path="./dataset/data_point_tensets_good.pk", random_seed=0, mode='test', test_group=test_group_idx, is_aug=False, inputSelect='FullPA')
